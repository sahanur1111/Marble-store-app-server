...
const getCartByEmail = async (req, res) => {
  try {
    ...
    res.status(200).json(result);// i forget the data to send 
  } catch (error) {
...
const addToCarts = async (req, res) => {
  const { name, product, image, price, email, quantity, menuItemId } = req.body;
...
const updateCart = async (req, res) => {
  ...
  try {
    const updatedCart = await Carts.findByIdAndUpdate(
      cartId,
      { name, product, image, price, email, quantity, menuItemId },
